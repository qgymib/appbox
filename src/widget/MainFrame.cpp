...
void MainFrame::StartPack(bool run_after)
{
    ...
    const auto range =
        static_cast<int>(std::min<std::size_t>(total, std::numeric_limits<int>::max()));
     ...
     */
    progress_dialog_ =
        new wxProgressDialog(run_after ? "Building and Running" : "Building",
                             "Preparing the archive...", range, this,
    ...
    pack_thread_ = std::thread([this, snapshot, loader_bytes, zip_wide, range, run_after]() {
        PackOutcome outcome;
        outcome.error = appbox::Pack(
            snapshot, loader_bytes.data(), loader_bytes.size(), zip_wide,
            [this, range](std::size_t done, std::size_t total_files) {
                const auto shown = static_cast<int>(
                    std::min<std::size_t>(done, static_cast<std::size_t>(range)));
                auto* event = new wxThreadEvent(APPBOX_PACK_PROGRESS);
                event->SetInt(shown);
                event->SetExtraLong(static_cast<long>(total_files));
        ...
        if (outcome.error.empty() && run_after)
        {
            ...
            outcome.error = appbox::ExtractArchive(zip_wide, folder.wstring());
...
void MainFrame::OnPackProgress(wxThreadEvent& event)
{
    ...
    const auto done = event.GetInt();
    const auto total = static_cast<std::size_t>(event.GetExtraLong());
    const auto text = total > 0
                          ? wxString::Format("Packing files: %d / %u", done,
                                             static_cast<unsigned>(total))
                          : "Packing files...";

    /* The maximum of the dialog is reserved for the finished state. */
    const auto reported = done > 0 ? static_cast<std::size_t>(done) : 0;
    if (!progress_dialog_->Update(appbox::BuildProgressValue(reported, pack_range_), text))
    {
        pack_cancelled_ = true;
    }
}
...
void MainFrame::OnPackFinished(wxThreadEvent& event)
{
    ...
    if (outcome.error == appbox::kPackCancelledError)
    {
     ...
     * before the user dismissed it.
     */
    progress_dialog_->Update(pack_range_,
                             wxString::FromUTF8(appbox::BuildResultMessage(result, result_error)));
